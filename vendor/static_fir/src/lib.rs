...
macro_rules! impl_fir {
    ($name:ident, $sample:ty, $size:expr, $coefs:expr) => {
        ...
        impl Default for $name {
            fn default() -> Self {
                $name([::num_traits::Zero::zero(); $size])
            }
        }
...
impl<C: FIRCoefs> FIRFilter<C> {
    ...
    /// Iterate over the history of stored samples in storage order (unordered).
    #[inline]
...
/// Backward-compatible alias (p25.rs uses this name).
pub type FirFilter<C> = FIRFilter<C>;
