...
use bits::Dibit;
use error::P25Error;
use baseband::cqpsk::DibitReceiver;
...
pub struct MessageReceiver {
    /// Lower-level stream receiver (C4FM path).
    recv: DataUnitReceiver,
    /// Lower-level dibit receiver (CQPSK path).
    dibit_recv: DibitReceiver,
    /// Current state.
    state: State,
    stats: Stats,
}
...
impl MessageReceiver {
    ...
    pub fn resync(&mut self) {
        self.recv.resync();
        self.dibit_recv.resync();
    }

    /// Feed in a baseband sample (C4FM path), possibly producing a message event.
    ...
    /// Feed a decoded dibit (CQPSK path), possibly producing a message event.
    pub fn feed_dibit(&mut self, dibit: Dibit) -> Option<MessageEvent> {
        match self.handle_dibit(dibit) {
            StateChange::Event(e) => Some(e),
            StateChange::EventChange(e, s) => {
                self.state = s;
                Some(e)
    ...
    /// Process the given sample and determine how to update state (C4FM path).
    ...
    /// Process the given dibit and determine how to update state (CQPSK path).
    /// Mirrors handle() but routes through dibit_recv instead of recv.
    fn handle_dibit(&mut self, d: Dibit) -> StateChange {
        use self::State::*;
        use self::StateChange::*;
        use message::nid::DataUnit::*;

        let event = match self.dibit_recv.feed(d) {
            Some(Ok(event)) => event,
            Some(Err(err)) => {
                self.dibit_recv.resync();
                return Event(MessageEvent::Error(err));
        ...
        self.stats.merge(&mut self.dibit_recv);
